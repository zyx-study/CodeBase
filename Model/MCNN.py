...
class MoireCNN(nn.Module):
    ...
    def __init__(self):
        ...
        self.s11 = nn.Sequential(  # 编码器模块 提取图像特征
            nn.Conv2d(3, 32, 3, 1, 1),
            nn.ReLU(True),
            nn.Conv2d(32, 32, 3, 1, 1)
        )
        self.s12 = self.up_conv_block()  # 解码器模块 反卷积 还原图像
        self.s13 = self.conv_block(32)  # 卷积块 提取高级别特征
...
class MoireCNN_2(nn.Module):

    def conv_block(self, channels):
        convs = [nn.Conv2d(channels, channels, 3, 1, 1) for _ in range(5)]
        relus = [nn.ReLU(True) for _ in range(5)]
        layers = [layer for pair in zip(convs, relus) for layer in pair]
...
if __name__ == '__main__':
    ...
    summary(model, input_shape=(3, 256, 256))
