...
img_align_celeba = os.path.join(root, 'image', 'new')  # 设置图片路径

parser = argparse.ArgumentParser()
parser.add_argument("--epoch", type=int, default=0)  # 开始训练的epoch 大于0将读取之前保存的模型继续训练
parser.add_argument("--n_epochs", type=int, default=100)  # 训练多少epoch
parser.add_argument("--dataset_name", type=str, default=img_align_celeba)  # 图像数据路径
parser.add_argument("--batch_size", type=int, default=1)  # batch_size大小 默认为4
...
parser.add_argument("--sample_interval", type=int, default=10)  # 保存输出图像（效果图与指标曲线）的batch间隔
