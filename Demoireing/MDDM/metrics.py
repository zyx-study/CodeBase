...
# 计算图像
def ciede2000_values(image1, image2):
    image1 = cv2.cvtColor(image1, cv2.COLOR_BGR2LAB).astype(float)
    image2 = cv2.cvtColor(image2, cv2.COLOR_BGR2LAB).astype(float)

    ciede2000_values = np.zeros((image1.shape[:2]), float)
    for i in range(image1.shape[0]):
        for j in range(image1.shape[1]):
            ciede2000_values[i, j] = CIEDE2000(image1[i, j], image2[i, j])

    return ciede2000_values.mean().item()
